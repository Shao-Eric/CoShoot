...
import { View, Text, FlatList, TouchableOpacity, Image, Platform, Share } from 'react-native';
        ...
        return (
            <View style={{ backgroundColor: 'white', flex: 1 }}>
                ...
                >
                    ...
                    {this.state.roomCode && <TouchableOpacity
                        style={{ alignItems: 'flex-end', margin: 10, borderRadius: 24 }}
                        onPress={() => Share.share({
                            message: this.state.roomCode
                        })}
                    >
                        <Text style={{ fontSize: 18, padding: 8, color: 'white' }}>Share Room Code</Text>
                    </TouchableOpacity>}
                </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) =>
                        <View style={{ flexDirection: 'row', margin: 12, justifyContent: 'space-between' }}>
                            <Text>{item.name}</Text>
                            {this.newRoom && ((this.state.first.uid === item.uid) ? <Text >First Shooter</Text> : <TouchableOpacity
                                style={{ justifyContent: 'flex-end', borderRadius: 24 }}
                                onPress={() => this.setState({ first: item }, () => console.log(this.state.first.uid))}
                            >
                ...
                {this.newRoom ? <TouchableOpacity
                    onPress={() => this.context.emit('start', this.state.first)}
                    style={{ justifyContent: 'flex-end', alignItems: 'center', margin: 4, borderRadius: 16, padding: 12, borderWidth: 2, borderColor: '#ff1744' }}>
                    <Text style={{ fontSize: 18, color: '#ff1744' }}>Start Shooting</Text>
                </TouchableOpacity> : <View style={{ justifyContent: 'flex-end', alignItems: 'center', margin: 4, borderRadius: 16, padding: 12 }}>
                        <Text style={{ fontSize: 18, color: '#ff1744' }}>Waiting to start...</Text>
                    </View>}
            </View>
        );
    }
